...
- **알림이 실제로 뜨는지 확인**: 저녁 8시까지 기다리지 않아도 되도록, 같은 자리에 **"🔔 [개발용] 5초 뒤 테스트 알림 보내기"** 버튼도 추가했습니다(`DevNotificationTestButton`, `sendTestNotificationIn`). 누르면 알림 권한을 요청하고, 5초 뒤 테스트 알림이 실제로 뜨는지 그 자리에서 바로 확인할 수 있습니다.
