...
`users` + 각 유저의 `bookProgress` + `bookParticipants`(참여 기록) + `cheerLogs`(화이팅 기록) + `pokeLogs`(찌르기 기록) + `webPushSubscriptions`(웹 푸시 구독)를 전부 삭제합니다. `books` 컬렉션은 건드리지 않습니다.
...
- **스트릭/유예(streakDays/graceDaysLeft)**: 책 단위가 아니라 유저 전역 기준이고, 실제 Firestore 값은 **"읽었어요!"에서만** 갱신됩니다("N장 더 읽었어요!"는 밀린 걸 갚는 것뿐이라 스트릭에 영향 없음). 어제 이어서 읽으면 연속 기록 +1, 하루 이상 건너뛰면 남은 유예일로 커버를 시도하고, 유예를 초과하면 스트릭이 1로 리셋됩니다. 매일 자정 서버가 따로 계산하지 않아도 되도록, 화면에 보여줄 때는 `computeLiveStreakDays`/`computeLiveOverdueChapters`가 그 자리에서 실시간으로 다시 계산합니다(아래 참고).
  - **유예(graceDaysLeft)는 가입할 때 한 번(2일) 주어지고, 그 뒤로는 절대 다시 채워지지 않습니다.** 연속으로 잘 읽어도 회복되지 않고, 스트릭이 끊겨서 1로 리셋돼도 유예가 2로 재지급되지 않으며, 66권을 완독하고 새 회독을 시작해도 마찬가지입니다 — 소모(밀린 날만큼 차감)만 있고 충전은 없습니다. 며칠 건너뛸 때마다 남은 유예에서 그만큼을 빼고, 유예가 0이 되고 나면 그다음부터는 하루만 밀려도 바로 스트릭이 끊깁니다.
  - 가입 직후(`graceDaysLeft` 기본값 2, `overdueChapters` 0)는 첫 "읽었어요!" 이후와 동일한 상태라, **가입 첫날에는 아래 스트릭 경고 배너가 뜨지 않습니다.**
  - **유예(대개 2일)를 넘도록 계속 안 읽으면(`isGraceExpired`)**, 다음 "읽었어요!"를 누르기 전이라도 **화면에는 이미 스트릭이 0으로 표시됩니다**(`computeLiveStreakDays`가 화면을 열 때마다 그 자리에서 판단). 실제 Firestore 값은 그대로 있다가, 다음 "읽었어요!"를 누르는 순간 `recordChaptersRead`가 스트릭을 1로 리셋하고 그동안의 공백을 원금에 확정합니다. **밀린 장수는 스트릭과 무관하게 계속 실시간으로 누적 표시됩니다** — 유예를 넘겼다고 화면에서 0으로 감춰버리면 정상처럼 보이다가 "읽었어요!"를 누르는 순간에야 갑자기 튀어나오는 문제가 있어서, 스트릭 상태와 상관없이 항상 그 자리에서 계산한 값을 그대로 보여주도록 바꿨습니다.
- **스트릭 경고 배너(`StreakWarningBanner`)**: 밀린 장수가 있을 때만 읽기 화면 상단에 `🔥 끊어진 불꽃을 다시 태울 수 있는 기회! {n}일 남았습니다!`(유예 소진 시에는 `오늘까지입니다!`)를 표시합니다. 남은 유예일수도 `computeLiveGraceDaysLeft`로 실시간 계산해서 넘겨줍니다(유예 초과 시 0). 밀린 장수가 0이 되면(다 따라잡았든, 정말 처음부터 밀린 게 없든) 곧바로 사라집니다.
- **밀린 장수 계산 (`computeLiveOverdueChapters`)**: 스트릭/유예 상태와 무관하게, 세 요소를 더해서 화면을 열 때마다 그 자리에서 다시 계산합니다.
...
- `users` 컬렉션 전체를 **실시간(onSnapshot)** 구독하고, 클라이언트에서 다음 기준으로 정렬합니다(Firestore 복합 색인이 필요 없도록 서버 정렬 대신 클라이언트 정렬을 씁니다).
  ...
  3. 진행률까지 같으면 **그 진행률을 먼저 달성한 사람이 위로** — `totalProgressPercent`가 바뀔 때마다 함께 기록되는 `progressUpdatedAt`(오름차순, 더 이른 사람이 우선)으로 가립니다. 마이그레이션 이전 유저처럼 이 필드가 없으면 가입 시각(`createdAt`)으로 대체합니다.
...
- **아이콘**(`assets/icon.png`, `android-icon-foreground/background/monochrome.png`, `favicon.png`): 네이비 원형 안에 책+길 심볼과 "Bible Road" 워드마크가 들어간 디자인으로 교체했습니다(GitHub 웹 업로드로 받은 원본을 Pillow로 안전 영역에 맞게 재가공). 업로드 원본을 그대로 쓰지 않고 아래처럼 다시 처리했습니다:
  - `icon.png`/`favicon.png`: 원본이 캔버스의 ~67%만 채우고 있어 실기기에서 작아 보이는 문제가 있어 96% 채움으로 재스케일(`favicon.png`은 최종 `icon.png`을 196×196으로 축소해 생성, 둘이 픽셀 단위로 어긋나지 않도록).
  - `android-icon-foreground.png`/`android-icon-monochrome.png`: 원본이 ~43~52%만 채워 안드로이드에서 오히려 너무 작아 보였던 문제를 58% 채움(세이프존 안쪽)으로 재스케일.
  - `android-icon-background.png`: 원본이 둥근 사각형 모양 그 자체를 그림 안에 그려놓은 형태(가장자리가 투명)였는데, adaptive icon의 background 레이어는 안드로이드가 알아서 원형/스퀴클 등으로 마스킹하기 때문에 **가장자리까지 꽉 채운 사각형(둥근 모서리 없이)** 이어야 합니다. 그라데이션 안쪽 영역만 잘라내 1024×1024 전체를 채우도록 다시 만들었습니다 — 안 그러면 원형 마스크를 씌웠을 때 네 귀퉁이에 투명한 틈이 생깁니다.
  - 원형 마스크를 시뮬레이션해서 전경 텍스트/심볼이 잘리지 않는지 합성 이미지로 직접 확인 후 반영했습니다.
  - `public/apple-touch-icon.png`: iOS는 이 아이콘에 **투명 배경(alpha)을 지원하지 않고**, 그대로 두면 투명한 영역을 예측 불가능한 색(보통 검정)으로 채워버려 "아이콘 크기/여백이 이상해 보이는" 문제가 생깁니다. `icon.png`를 그대로 리사이즈하는 대신, `android-icon-background.png`와 같은 네이비 그라데이션 위에 `icon.png`를 합성한 뒤 **완전히 불투명하게(alpha 채널 제거)** 만들어서 생성합니다.
  - `public/icon-maskable-192.png`/`icon-maskable-512.png`(+ `manifest.json`의 `purpose: "maskable"` 항목): 안드로이드에서 PWA를 홈 화면에 설치할 때, 매니페스트에 `purpose: "any"` 아이콘만 있으면 크롬이 adaptive icon 마스킹을 위해 아이콘을 축소하고 여백을 넣어버려서 "안드로이드 아이콘이 유독 작아 보이는" 문제가 있었습니다. `android-icon-background.png`(가장자리까지 꽉 채운 배경) + `android-icon-foreground.png`(세이프존 안쪽 전경)를 합성한 전용 maskable 아이콘을 추가해서, 안드로이드가 알아서 마스킹해도 로고가 작아지지 않도록 했습니다.
