...
- **밀린 장수(overdueChapters)**: `max(0, 가입일 이후 "완전히 지난 날" 수 × DAILY_CHAPTER_GOAL − 지금까지 읽은 전체 장수)`로 계산합니다. 가입 당일은 아직 하루가 안 지났으므로 항상 0입니다. **화면을 열 때마다 그 자리에서 다시 계산**합니다(저장된 값을 그대로 믿지 않음) — 그래야 아무 액션 없이 며칠이 지나도 값이 늘어난 게 정확히 보입니다. `users` 문서의 `overdueChapters` 필드 자체는 읽기 액션 이후의 스냅샷으로만 남아있고(추후 다른 화면/랭킹 등에서 참고용), 읽기 화면 표시에는 쓰이지 않습니다.
